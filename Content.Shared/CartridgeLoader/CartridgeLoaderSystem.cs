...
public sealed partial class CartridgeLoaderSystem : EntitySystem
{
    ...
    {
        if (ent.Comp.ActiveProgram.HasValue)
        {
            var ev = new CartridgeLoaderExternalContainerChangedEvent(ent, args.Container.ID, args.Entity, true);
            RaiseLocalEvent(ent.Comp.ActiveProgram.Value, ref ev);
        }
    ...
    private void OnItemRemoved(Entity<CartridgeLoaderComponent> ent, ref EntRemovedFromContainerMessage args)
    {
        if (ent.Comp.ActiveProgram.HasValue)
        {
            var ev = new CartridgeLoaderExternalContainerChangedEvent(ent, args.Container.ID, args.Entity, false);
            RaiseLocalEvent(ent.Comp.ActiveProgram.Value, ref ev);
        }
