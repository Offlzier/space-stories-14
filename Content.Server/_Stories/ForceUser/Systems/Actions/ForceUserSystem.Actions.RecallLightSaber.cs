...
public sealed partial class ForceUserSystem
{
    ...
    private void OnTimeOut(EntityUid uid, LightsaberComponent comp, PulledToTimeOutEvent args)
    {
        ...
        _popup.PopupEntity(
            Loc.GetString(_hands.TryPickupAnyHand(args.Component.PulledTo.Value, uid)
                ? "Ваш световой меч телепортируется вам в руку!"
                : "ninja-hands-full"),
            args.Component.PulledTo.Value,
            args.Component.PulledTo.Value);
    }
    ...
    private void OnRecall(EntityUid uid, ForceUserComponent comp, RecallLightsaberEvent args)
    {
        ...
        if (_container.IsEntityInContainer(comp.Lightsaber.Value) &&
