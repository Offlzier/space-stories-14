...
public sealed partial class ForceUserSystem
{
    ...
    private void OnLookUp(EntityUid uid, ForceUserComponent component, ForceLookUpActionEvent args)
    {
        ...
        foreach (var ent in ents)
        {
            if (ents.Count == 1 && ent.Owner == uid)
                ...
                continue;
            else if (ents.Count == 2)
                _popup.PopupEntity(Loc.GetString("force-lookup-one", ("name", ent.Comp.Name())), uid, uid);
            else if (ents.Count > 2)
                _popup.PopupEntity(Loc.GetString("force-lookup-many"), uid, uid);
            break;
        }
    ...
    private void OnFlashAreaEvent(FlashAreaEvent args)
    {
        if (args.Handled)
            return;
        _flashSystem.FlashArea(args.Performer,
            args.Performer,
            args.Range,
            TimeSpan.FromMilliseconds(args.FlashDuration),
            args.SlowTo,
            sound: args.Sound);
        args.Handled = true;
    }
    ...
    private void OnEmp(EmpActionEvent args)
    {
        if (args.Handled)
            return;
        _emp.EmpPulse(_transform.GetMapCoordinates(args.Performer),
            args.Range,
            args.EnergyConsumption,
            TimeSpan.FromSeconds(args.DisableDuration));
        args.Handled = true;
    }
}
