...
public sealed partial class ForceUserSystem
{
    ...
    {
        if (!HasComp<TetheredComponent>(uid) || args.DamageDelta == null)
            return;
    ...
    {
        if (component.LightsaberOwner != null && args.User != component.LightsaberOwner &&
            HasComp<TetheredComponent>(uid))
        {
            args.Cancel();
            return;
        }
